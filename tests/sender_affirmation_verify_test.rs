...
fn test_sender_affirmation_verify() {
    ...
    let issuer_acct = issuer.create_and_register_account(&mut rng, &mut chain, "1").unwrap();
    let investor1_acct = investor1.create_and_register_account(&mut rng, &mut chain, "1").unwrap();

    let asset_id = issuer.create_asset(&mut chain, &[], &[]).unwrap();
    let asset = chain.get_asset_state(asset_id).unwrap();

    issuer_acct.initialize_asset(&mut rng, &mut chain, asset_id).unwrap();
    investor1_acct.initialize_asset(&mut rng, &mut chain, asset_id).unwrap();
    ...
    let settlement = SettlementBuilder::new(b"Test")
        ...
        .encrypt_and_prove(&mut rng, chain.asset_tree()).unwrap();
    
    let settlement_id = issuer.create_settlement(&mut chain, settlement.clone()).unwrap();
    ...
    let (_, leg_enc_rand) = leg_enc.decrypt_with_randomness(LegRole::Sender, &issuer_acct.keys()).unwrap();
    ...
    proof.verify(&leg_enc, chain.account_tree(), &mut rng).unwrap();
    
    println!("For L={ACCOUNT_TREE_L}, height={ACCOUNT_TREE_HEIGHT}, verification time: {:?}", start.elapsed());
}
