...
fn test_minting_asset() -> Result<()> {
    ...
    let asset_details = chain.create_dart_asset(issuer, AuditorOrMediator::mediator(&mediator_keys.public_keys()))?;
    let asset_id = asset_details.asset_id;

    // Initialize account asset state for the issuer and investor.
    let (proof, mut issuer_account) = AccountAssetRegistrationProof::new(&mut rng, &issuer_keys, asset_id);
    chain.initialize_account_asset(issuer, proof)?;
    issuer_account.commit_pending_state();
    let (proof, mut investor1_account) = AccountAssetRegistrationProof::new(&mut rng, &investor1_keys, asset_id);
    ...
    let proof = AssetMintingProof::new(&mut rng, &mut issuer_account, chain.prover_account_tree(), 1000)?;
