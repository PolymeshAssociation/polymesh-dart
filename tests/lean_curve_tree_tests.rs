...
use polymesh_dart::curve_tree::{
    LeanCurveTree, CompressedCurveTreeRoot,
    AccountTreeConfig, CurveTreeRoot, FullCurveTree, LeafValue,
};
...
fn setup_trees() -> (
    ...
    let mut lean_tree = LeanCurveTree::<L, 1, AccountTreeConfig>::new(HEIGHT, &params, &mut compressed_root)
    ...
    lean_tree.append_leaf(leaf, &params, &mut compressed_root).unwrap();
    storage_tree.insert(leaf).unwrap();

    // Compare roots
    let lean_root = compressed_root.root_node::<AccountTreeConfig>().expect("Failed to get compressed root");
    //let lean_root = lean_tree.root_node(&params)
    //    .expect("Failed to get lean tree root");
...
fn test_inserts() {
    ...
    for i in 1..=5 {
        ...
        // Compare roots
        let lean_root = compressed_root.root_node().expect("Failed to get full root");
        let storage_root = storage_tree
            .root_node()
            .expect("Failed to get storage root");
