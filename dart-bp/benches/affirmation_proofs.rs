...
use polymesh_dart_bp::account::{AccountCommitmentKeyTrait, AccountState, AffirmAsSenderTxnProof, AffirmAsReceiverTxnProof};
use polymesh_dart_bp::keys::{keygen_sig, keygen_enc, SigKey, VerKey, DecKey, EncKey};
use polymesh_dart_bp::leg::{Leg, LegEncryption, LegEncryptionRandomness};
use polymesh_dart_bp::poseidon_impls::poseidon_2::params::{Poseidon2Params, pallas::{MAT_DIAG3_M_1, MAT_INTERNAL3, RC3}};
...
fn create_shared_setup<R: rand_core::CryptoRngCore>(rng: &mut R) -> (SelRerandParameters<PallasParameters, VestaParameters>, [PallasA; 8], PallasA, PallasA) {
    const NUM_GENS: usize = 1 << 12;
    
    let account_tree_params = SelRerandParameters::<PallasParameters, VestaParameters>::new(NUM_GENS, NUM_GENS).unwrap();
...
fn create_poseidon_config() -> Poseidon2Params<Fr> {
    ...
    let degree = 5;
    Poseidon2Params::<Fr>::new(3, degree, full_rounds, partial_rounds, 
        MAT_DIAG3_M_1.as_ref().unwrap().to_vec(), 
        MAT_INTERNAL3.as_ref().unwrap().to_vec(), 
        RC3.as_ref().unwrap().to_vec()).unwrap()
}
...
fn create_keys<R: rand_core::CryptoRngCore>(
    ...
    enc_key_gen: PallasA,
) -> ((SigKey<PallasA>, VerKey<PallasA>), (DecKey<PallasA>, EncKey<PallasA>), (SigKey<PallasA>, VerKey<PallasA>), (DecKey<PallasA>, EncKey<PallasA>), (DecKey<PallasA>, EncKey<PallasA>)) {
    ...
    ((sk_s, pk_s), (sk_s_e, pk_s_e), (sk_r, pk_r), (sk_r_e, pk_r_e), (sk_a_e, pk_a_e))
}
...
fn create_leg_and_encryption<R: rand_core::CryptoRngCore>(
    ...
    leg.encrypt::<_, Blake2b512>(rng, pk_s_e.0, pk_r_e.0, enc_key_gen, enc_gen).unwrap()
}
...
fn create_account_and_tree<R: rand_core::CryptoRngCore>(
...
) -> (AccountState<PallasA>, CurveTree<512, 1, PallasParameters, VestaParameters>) {
    let asset_id = 1;
    let id = Fr::rand(rng);
    ...
    let account_tree = CurveTree::<512, 1, PallasParameters, VestaParameters>::from_leaves(
        &set, &account_tree_params, Some(4)
    );
...
fn bench_sender_affirmation_verification(c: &mut Criterion) {
    ...
    let (account_tree_params, account_comm_key, enc_key_gen, enc_gen) = create_shared_setup(&mut rng);
    ...
    let (leg_enc, leg_enc_rand) = create_leg_and_encryption(
        &mut rng, pk_s, pk_r, pk_s_e, pk_r_e, pk_a_e, enc_key_gen, enc_gen
    );
    
    let (account, account_tree) = create_account_and_tree(&mut rng, sk_s, account_comm_key, &account_tree_params);
...
fn bench_receiver_affirmation_verification(c: &mut Criterion) {
    ...
    let (account, account_tree) = create_account_and_tree(&mut rng, sk_r, account_comm_key, &account_tree_params);
