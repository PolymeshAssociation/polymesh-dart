    ...
    RegTxnProof<G, CHUNK_BITS, NUM_CHUNKS>
{
    ...
    pub fn new<R: CryptoRngCore>(
        ...
        let (comm_rho_bp, mut vars) = prover.commit_vec(
        ...
        let (_, _, var_rho_sq_1) = prover.multiply(lc_rho.clone(), lc_rho.clone());
        prover.constrain(lc_rho_1 - lc_rho);
        prover.constrain(var_rho_sq_1 - var_rho_sq);
    ...
    pub fn verify<R: CryptoRngCore>(
        ...
        let _ = Self::CHECK_CHUNK_BITS;
        assert_eq!(T.is_none() ^ self.encrypted_randomness.is_none(), false);
        ...
        let mut vars = verifier.commit_vec(3, self.comm_rho_bp);
        let var_sk = vars.remove(0);
        let var_rho = vars.remove(0);
        let var_rho_sq = vars.remove(0);

        let lc_rho: LinearCombination<G::ScalarField> = var_rho.into();
        let c = LinearCombination::from(AccountState::<G>::concat_asset_id_counter(
            asset_id, counter,
        ));
        let lc_rho_1 = Poseidon_hash_2_constraints_simple(
            &mut verifier,
        ...
        let (_, _, var_rho_sq_1) = verifier.multiply(lc_rho.clone(), lc_rho.clone());
        verifier.constrain(lc_rho_1 - lc_rho);
        verifier.constrain(var_rho_sq_1 - var_rho_sq);
...
pub mod tests {
    ...
    fn account_init() {
        ...
        let (account, c, _, _) = new_account::<_, PallasA>(&mut rng, asset_id, sk_i);
        assert_eq!(account.asset_id, asset_id);
        assert_eq!(account.balance, 0);
        assert_eq!(account.counter, 0);
        // TODO: Check rho correctly generated
