...
use crate::util::{
    ...
    prove_with_rng,
    verify_given_verification_tuples, BPProof,
};
...
> SettlementTxnProof<L, F0, F1, G0, G1>
{
    ...
    pub fn new_with_given_prover<R: CryptoRngCore>(
        ...
        let (re_randomized_path, mut re_randomization_of_leaf) = leaf_path.select_and_rerandomize_prover_gadget(
            even_prover,
            odd_prover,
            tree_parameters,
            rng,
    ...
    // pub fn verify_with_given_verifier<R: CryptoRngCore>(
    //     &self,
    //     leg_enc: LegEncryption<Affine<G0>>,
    //     tree_root: &Root<L, 1, G1, G0>,
    //     nonce: &[u8],
    ...
    //     enc_gen: Affine<G0>,
    //     rng: &mut R,
    //     even_verifier: Verifier<MerlinTranscript, Affine<G1>>,
    //     odd_verifier: Verifier<MerlinTranscript, Affine<G0>>,
    //     mut rmc: Option<(
    //         &mut RandomizedMultChecker<Affine<G1>>,
    //         &mut RandomizedMultChecker<Affine<G0>>,
    //     )>,
    ...
    //         None => None,
    //     };
    //     let (even_tuple, odd_tuple) = self.verify_and_return_tuples_with_given_verifier(
    //         leg_enc,
    //         tree_root,
    //         nonce,
    //         tree_parameters,
    ...
    pub fn verify_and_return_tuples<R: CryptoRngCore>(
        ...
        // self.verify_and_return_tuples_with_given_verifier(
        //     leg_enc,
        //     tree_root,
        //     nonce,
        //     tree_parameters,
        ...
        let r1cs_proof = self.r1cs_proof.as_ref().ok_or_else(|| {
    ...
    //     enc_gen: Affine<G0>,
    //     rng: &mut R,
    //     mut even_verifier: Verifier<MerlinTranscript, Affine<G1>>,
    //     mut odd_verifier: Verifier<MerlinTranscript, Affine<G0>>,
    //     rmc: Option<&mut RandomizedMultChecker<Affine<G0>>>,
    // ) -> Result<(VerificationTuple<Affine<G1>>, VerificationTuple<Affine<G0>>)> {
    //     self.verify_sigma_protocols_and_enforce_constraints(
    //         leg_enc,
    ...
    //     )?;
    //
    //     let bp_proof = self.r1cs_proof.as_ref().ok_or_else(|| {
    //         Error::ProofVerificationError("R1CS proof is missing".to_string())
    //     })?;
    //     get_verification_tuples_with_rng(
    //         even_verifier,
    ...
    pub fn verify_sigma_protocols_and_enforce_constraints(
        ...
        // TODO: This is suboptimal if the same root is being used since the same children (of root) are being allocated each time this is called.
        let _ = self.re_randomized_path.select_and_rerandomize_verifier_gadget(
            tree_root,
            even_verifier,
            odd_verifier,
            &tree_parameters
...
pub mod tests {
    ...
    fn combined_leg_verification() {
        ...
        let (even_bp, odd_bp) = prove_with_rng(even_prover, odd_prover, &asset_tree_params, &mut rng).unwrap();
        ...
        println!("Combined leg RMC verification time = {:?}", rmc_verification_time);
        println!("Combined proof size = {} bytes", even_bp.compressed_size() + odd_bp.compressed_size() + proofs.compressed_size());
    }
