...
pub const POSEIDON2_CONSTANTS: usize = 3;
pub const POSEIDON2_MAT_INTERNAL_DIAG_M_1: usize = 3;

#[derive(Clone, Debug)]
//#[cfg(not(feature = "low_memory"))]
pub struct Poseidon2Constants<F: PrimeField> {
    constants: [F; POSEIDON2_CONSTANTS],
}

//#[cfg(not(feature = "low_memory"))]
impl<F: PrimeField> Poseidon2Constants<F> {
    pub fn new(constants_hex: &'static [&'static str; POSEIDON2_CONSTANTS]) -> Result<Self> {
        let mut temp = [0u8; 32];
        let mut constants = [F::zero(); POSEIDON2_CONSTANTS];
        for i in 0..POSEIDON2_CONSTANTS {
            let hex_str = constants_hex[i];
            if hex_str.len() > 0 {
                constants[i] = from_hex::<F>(hex_str, &mut temp)?;
            }
        }
        Ok(Self { constants })
    }

    pub fn constant(&self, idx: usize) -> F {
        self.constants[idx]
    }

    pub fn constants<'f>(
        &self,
        temp: &'f mut [F; POSEIDON2_CONSTANTS],
    ) -> &'f [F; POSEIDON2_CONSTANTS] {
        *temp = self.constants;
        temp
    }
}
...
#[derive(Clone, Debug)]
#[cfg(not(feature = "low_memory"))]
pub struct Poseidon2RoundConstants<F: PrimeField> {
    rounds: Vec<Poseidon2Constants<F>>,
}

#[cfg(not(feature = "low_memory"))]
impl<F: PrimeField> Poseidon2RoundConstants<F> {
    pub fn new_round_constants(
        rounds_hex: &'static [[&'static str; POSEIDON2_CONSTANTS]],
    ) -> Result<Self> {
        let mut rounds = Vec::with_capacity(rounds_hex.len());
        for round_hex in rounds_hex {
            rounds.push(Poseidon2Constants::new(round_hex)?);
        }
        Ok(Self { rounds })
    }

    pub fn round_constant(&self, round: usize, idx: usize) -> F {
        self.rounds[round].constant(idx)
    }

    pub fn round_constants<'f>(
        &self,
        round: usize,
        temp: &'f mut [F; POSEIDON2_CONSTANTS],
    ) -> &'f [F; POSEIDON2_CONSTANTS] {
        self.rounds[round].constants(temp)
    }
}
...
impl<F: PrimeField> Poseidon2RoundConstants<F> {
    ...
    pub fn round_constant(&self, round: usize, idx: usize) -> F {
        let mut temp = [0u8; 32];
        let round_hex = self.rounds_hex[round];
        from_hex::<F>(round_hex[idx], &mut temp).expect("Failed to decode round constant from hex")
    }

    pub fn round_constants<'f>(
        &self,
        round: usize,
        temp_constants: &'f mut [F; POSEIDON2_CONSTANTS],
    ) -> &'f [F; POSEIDON2_CONSTANTS] {
        let mut temp = [0u8; 32];
        let round_hex = self.rounds_hex[round];
        for i in 0..POSEIDON2_CONSTANTS {
            temp_constants[i] = from_hex::<F>(round_hex[i], &mut temp)
                .expect("Failed to decode round constant from hex");
        }
        temp_constants
    }
}
...
pub struct Poseidon2Params<F: PrimeField> {
    ...
    pub rounds: usize,
    round_constants: Poseidon2RoundConstants<F>,
}
...
impl<F: PrimeField> Poseidon2Params<F> {
    ...
    pub fn round_constant(&self, round: usize, idx: usize) -> F {
        self.round_constants.round_constant(round, idx)
    }
    ...
    pub fn round_constants<'f>(
        ...
        self.round_constants.round_constants(round, temp)
    }
}
...
pub mod pallas {
    ...
    fn get_round_constants() -> Result<Poseidon2RoundConstants<PallasFr>> {
        Ok(Poseidon2RoundConstants::new_round_constants(&[
            [
