...
4. Verifies correctness of $State_{old_r}$ by checking:
   ...
   $$\implies (bal_{1_{blinding}} + bal_1.c).G_1 + (\rho_{blinding} + \rho.c).G_5 + (s_{blinding} + s.c).G_6 + $$
   $$(b_{0_{blinding}} + b_0.c).H_0 \stackrel{?}{=} T_{State_{old_r}} + (State_{old_r} + v.G_1 - AK - at.G_3).c$$
...
3. Verifier enforces the Bulletproof constraints for curve tree membership to verify that $Path_r$ leads to $Root$. (Constraints in the appendix)
4. Verifies correctness of $State_{old_r}$ by checking:
   $$Resp_{state_{old_r}}[0].G_{Aff} + Resp_{state_{old_r}}[1].G_1 + Resp_{state_{old_r}}[3].G_5 + Resp_{state_{old_r}}[4].G_6 + $$
   $$Resp_{state_{old_r}}[5].H_0 \stackrel{?}{=} T_{State_{old_r}} + (State_{old_r} + v.G_1 - at.G_3).c$$

   $$\implies (sk_{blinding} + sk.c).G_{Aff} + (bal_{1_{blinding}} + bal_1.c).G_1 + (\rho_{blinding} + \rho.c).G_5 + (s_{blinding} + s.c).G_6 + $$
   $$(b_{0_{blinding}} + b_0.c).H_0 \stackrel{?}{=} T_{State_{old_r}} + (State_{old_r} + v.G_1 - at.G_3).c$$
...
5. Verifies correctness of $State_{new}$ by checking:
   ...
   $$\implies (sk_{blinding} + sk.c).G_{Aff} + (bal_{1_{blinding}} + bal_1.c).G_1 + (\rho'_{blinding} + \rho'.c).G_5 + $$
   $$(s'_{blinding} + s'.c).G_6 \stackrel{?}{=} T_{State_{new}} + (State_{new} - at.G_3).c$$
