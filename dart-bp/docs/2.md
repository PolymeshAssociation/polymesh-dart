...
10. Prover hashes the following to create challenge $c$ as:
    $$c = Hash(State_0, AK, EK, pk_{enc\_inv}, at, ctr, N, id, C_i, pk_T, Com_s, C_{BP_\rho}, C_{BP_s}, $$
...
11. Prover now creates responses for each sigma protocol as:
    ...
    $$resp_{pk\_enc} = ek_r + ek.c$$
    $$resp_{pk\_enc\_inv} = ek^{-1}_r + ek^{-1}.c$$
    Note that $resp_{enc\_key\_gen}$ (for proving $G_{Enc} = EK \cdot ek^{-1}$) reuses $resp_{pk\_enc\_inv}$ since the witness is the same $ek^{-1}$.
    $$resp_{r\_combined} = (r_{combined})_r + r_{combined} \cdot c$$
...
1. Verifier hashes the following to create challenge $c$ as:
   ...
   $$c = Hash(State_0, AK, at, ctr, N, id, C_i, pk_T, Com_s, C_{BP_\rho}, C_{BP_s}, $$
   $$T_{state}, T_{null}, T_{s-combined}, T_{enc_i}, T_{chunks_i}, T_{BP_\rho}, T_{BP_s}, T_{pk}, T_{pk\_enc}, T_{pk\_enc\_inv}, T_{enc\_key\_gen})
   $$
...
7. Verifies encryption key relations:
   $$resp_{pk\_enc}.G_{Enc} \stackrel{?}{=} T_{pk\_enc} + EK.c$$
   $$resp_{pk\_enc\_inv}.G_{Enc} \stackrel{?}{=} T_{pk\_enc\_inv} + pk_{enc\_inv}.c$$
   $$resp_{pk\_enc\_inv}.EK \stackrel{?}{=} T_{enc\_key\_gen} + G_{Enc}.c$$
   Note: the last check uses the same response $resp_{pk\_enc\_inv}$ as the previous one, which proves $ek^{-1}$ in $pk_{enc\_inv}$ is indeed the inverse of $ek$ in $EK$.
