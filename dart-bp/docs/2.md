...
4. Note that
   $$Com_s = \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot C_{i_1} = \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot r_{\mathrm{enc}_i} \cdot pk_T + s \cdot G$$
   $$Com_r = \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot C_{i_0} = \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot r_{\mathrm{enc}_i} \cdot G_{\mathrm{Enc}}$$
...
9. Prover proves knowledge of above committed values and equality of these with other commitments as:
   ...
   $$T_{BP_s} = r_{BP_s}.H_0 + s_{chunks-blinding_0}.H_1 + s_{chunks-blinding_1}.H_2 + ... + $$
   $$s_{chunks-blinding_{NUM\_CHUNKS-1}}.H_{NUM\_CHUNKS}, \in \mathbb{G}_p$$
...
11. Prover hashes the following to create challenge $c$ as:
    $$c = Hash(State_0, AK, at, c, N, id, C_i, pk_T, Com_s, Com_r, C_{BP_\rho}, C_{BP_s}, $$
...
12. Prover now creates responses for each sigma protocol as:
    ...
    $$Resp_r = r_{\mathrm{combined}} + \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot r_{\mathrm{enc}_i} \cdot c$$
...
7. Verifier creates $Com_r$ and $Com_s$ as:
   $$Com_r = \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot C_{i_0}$$
   $$Com_s = \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot C_{i_1}$$
8. Verifies correctness of $Com_r$ by checking:
   $$Resp_r.G_{enc} \stackrel{?}{=} T_{r-combined} + Com_r.c$$
   $$\implies (r_{\mathrm{combined}} + \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot r_{\mathrm{enc}_i} \cdot c) \cdot G_{\mathrm{enc}} \stackrel{?}{=} T_{r-\mathrm{combined}} + Com_r \cdot c$$
9. Verifies correctness of $Com_s$ by checking:
   $$Resp_r.pk_T + Resp_{state}[1].G \stackrel{?}{=} T_{s-combined} + Com_s.c$$
   $$\implies (r_{\mathrm{combined}} + \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot r_{\mathrm{enc}_i} \cdot c) \cdot pk_T + (s_{\mathrm{blinding}} + s \cdot c) \cdot G \stackrel{?}{=} T_{s-\mathrm{combined}} + Com_s \cdot c$$
...
13. Verifier checks response for $C_{BP_s}$ as:
    ...
    $$(s_{chunks-blinding_0} + s_{chunks_0}).H_1 + (s_{chunks-blinding_1} + s_{chunks_1}).H_2 + ... + $$
    $$(s_{chunks-blinding_{NUM\_CHUNKS-1}} + s_{chunks_{NUM\_CHUNKS-1}}).H_{NUM\_CHUNKS} \stackrel{?}{=} T_{BP_s} + C_{BP_s}.c$$
