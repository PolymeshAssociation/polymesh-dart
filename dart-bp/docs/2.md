...
4. Verifies correctness of $State_0$ by checking:
   ...
   $$\implies (\rho^2_r + \rho^2.c).G_5 + (s_r + s.c).G_6 \stackrel{?}{=} T_{state} + State_0.c$$
...
6. Verifies correctness of $N$ by checking:
   ...
   $$\implies (\rho_r + \rho.c).G_4 \stackrel{?}{=} T_{null} + N.c$$
...
8. Verifies correctness of $Com_r$ by checking:
   ...
   $$\implies (r_{combined} + \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot r_{\mathrm{enc}_i} \cdot c) \cdot G_{\mathrm{enc}} \stackrel{?}{=} T_{r-\mathrm{combined}} + Com_r \cdot c$$
9. Verifies correctness of $Com_s$ by checking:
   $$resp_r.pk_T + resp_{state}[1].G \stackrel{?}{=} T_{s-combined} + Com_s.c$$
   $$\implies (r_{combined} + \sum_{i} 2^{i \cdot \mathrm{CHUNK\_BITS}} \cdot r_{\mathrm{enc}_i} \cdot c) \cdot pk_T + (s_r + s \cdot c) \cdot G \stackrel{?}{=} T_{s-combined} + Com_s \cdot c$$
10. Verifier checks response for $C_{i,0}$ as:
    $$resp_{i,0}.G_{Enc} \stackrel{?}{=} T_{enc_i} + C_{i,0}.c$$
    $$\implies (r_{(\mathrm{enc}_i)_r} + r_{\mathrm{enc}_i}.c).G_{Enc} \stackrel{?}{=} T_{enc_i} + C_{i,0}.c$$
11. Verifier checks response for $C_{i,1}$ as:
    $$resp_{i,0}.pk_T + resp_{s_{\mathrm{chunks}_i}}.G \stackrel{?}{=} T_{chunks_i} + C_{i,1}.c$$  
    $$\implies (r_{(\mathrm{enc}_i)_r} + r_{\mathrm{enc}_i}.c).pk_T + (s_{(\mathrm{chunks}_i)_r} + s_{\mathrm{chunks}_i}.c).G \stackrel{?}{=} T_{chunks_i} + C_{i,1}.c$$
12. Verifier checks response for $C_{BP_\rho}$ as:
    ...
    $$\implies (r_{BP_\rho} + b.c).H_0 + (sk_r + sk.c).H_1 + (\rho_r + \rho.c).H_2 + (\rho^2_r + \rho^2.c).H_3 \stackrel{?}{=} T_{BP_\rho} + C_{BP_\rho}.c$$
13. Verifier checks response for $C_{BP_s}$ as:
    ...
    $$(s_{(\mathrm{chunks}_{NUM\_CHUNKS-1})_r} + s_{\mathrm{chunks}_{NUM\_CHUNKS-1}}.c).H_{NUM\_CHUNKS} \stackrel{?}{=} T_{BP_s} + C_{BP_s}.c$$
