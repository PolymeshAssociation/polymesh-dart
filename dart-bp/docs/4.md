...
1. Then it decrypts $CT_s, CT_r$ to get its public key and ensure that its the correct key
...
5. Now that there is a public $E_i$ corresponding to each public key $EK_i$ as $E_i = role_i.J + EK_i + bl_i.H_0$, we can state $EK_i$ in terms of $E_i$ as $E_i - role_i.J - bl_i.H_0 = EK_i$ since the LHS is a "blinded version" of $EK_i$ and is different for the same $EK_i$ in different proofs (as long as fresh $bl_i$ is chosen).
   ...
   $$
   which is doable using folklore techniques as all group elements, $E_i, H_0, J$ are public. In fact, $role_i.J$ is public since $role_i$ is public.
...
7. Verifies correctness of $AT_r$ by checking:
   ...
   $$\implies (at_r + at.c).J + (bl_{0_r} + bl_0.c).H_0 \stackrel{?}{=} at_r.J + bl_{0_r}.H_0 + AT_r.c$$
...
8. For each $i$, verifies correctness of $Eph_{k_i}[0]$ by checking:
   ...
   $$\implies (r_{1_r} + r_1.c).(E_i - role_i.J) + (bl_{i_r} + r_1.bl_i.c).(-H_0) \stackrel{?}{=} r_{1_r}.(E_i - role_i.J) + bl_{i_r}.(-H_0) + Eph_{k_i}[0].c$$
...
9. For each $i$, verifies correctness of $Eph_{k_i}[1]$ by checking:
   ...
   $$\implies (\alpha_r + \alpha.c).Eph_{k_i}[0] \stackrel{?}{=} \alpha_r.Eph_{k_i}[0] + Eph_{k_i}[1].c$$
...
10. For each $i$, verifies correctness of $Eph_{k_i}[2]$ by checking:
    ...
    $$\implies (\beta_r + \beta.c).Eph_{k_i}[0] \stackrel{?}{=} \beta_r.Eph_{k_i}[0] + Eph_{k_i}[2].c$$
...
11. For each $i$, verifies correctness of $Eph_{k_i}[3]$ by checking:
    ...
    $$\implies (\gamma_r + \gamma.c).Eph_{k_i}[0] \stackrel{?}{=} \gamma_r.Eph_{k_i}[0] + Eph_{k_i}[3].c$$
...
12. Verifier checks response for $C_{BP}$ as:
    ...
    $$(\alpha_r + \alpha.c).H_5 + (\beta_r + \beta.c).H_6 + (\gamma_r + \gamma.c).H_7 + (v_r + v.c).H_8 \stackrel{?}{=} r_{BP}.H_0 + r_{1_r}.H_1 + $$
    $$r_{2_r}.H_2 + r_{3_r}.H_3 + r_{4_r}.H_4 + \alpha_r.H_5 + \beta_r.H_6 + \gamma_r.H_7 + v_r.H_8 + C_{BP}.c$$
...
A mediator for an asset can either accept or reject a leg in a settlement. It needs to prove that it knows the secret key for the asset's associated mediator's public key.
Assets can have multiple mediators and each sends its own acceptance or rejection proof. 
It needs to prove the knowledge of (inverse of) secret key $sk_m^{-1}$ in relation $CT_{at} = Eph_{k_i}[3].sk_m^{-1} + at.(-H)$, since $Eph_{k_m}[3] = r_4.EK_m = r_4.sk_m.G_{Enc}$ and $CT_{at} = r_4.G_{Enc} + at.H$

**Instance**: $CT_{at}, Eph_{k_m}[3], accept/reject, G_{Enc}, H$
...
2. Let $D = Eph_{k_i}[3]$ be the ephemeral key created for this mediator in the leg encryption.
3. Prover wants to prove knowledge of $sk_m$ in the relation:
   $$D.sk_m^{-1} + at.(-H) = CT_{at}$$
   where $D, H, CT_{at}$ are public and $sk_m, at$ are witnesses.
4. Prover picks random $sk_{m_r}, at_r \in \mathbb{Z}_p$ and creates:
   $$T_{enc\_pk} = sk_{m_r}.D + at_r.(-H), \in \mathbb{G}_p$$
5. Prover hashes the following to create challenge $c$ as:
   $$c = Hash(accept/reject, Leg, m, D, CT_{at}, T_{enc\_pk})$$
   where $Leg$ is the leg encryption, $m$ is the mediator's position in the asset's auditor/mediator list
6. Prover creates responses:
   $$resp_{sk_m} = sk_{m_r} + sk_m^{-1}.c$$
   $$resp_{at} = at_r + at.c$$
7. The proof is
   $$(T_{enc\_pk}, resp_{sk_m}, resp_{at})$$

### Verifier

1. Verifier first checks that $m$ corresponds to a mediator (not auditor) in the leg encryption.
2. Verifier adds the mediator's acceptance or rejection to the transcript.
3. Let $D = Eph_{k_i}[3]$ be the ephemeral key at position $m$ in the leg encryption.
4. Verifier reconstructs the challenge $c$ as:
   $$c = Hash(accept/reject, Leg, m, nonce, D, CT_{at}, T_{enc\_pk})$$
5. Verifies correctness of the opening by checking:
   $$resp_{sk_m}.D + resp_{at}.(-H) \stackrel{?}{=} T_{enc\_pk} + CT_{at}.c$$
   $$\implies (sk_{m_r} + sk_m^{-1}.c).D + (at_r + at.c).(-H) \stackrel{?}{=} sk_{m_r}.D + at_r.(-H) + CT_{at}.c$$
