...
6. Prover picks random $sk_r, cnt_r, bal_{0_r}, \rho_r, \rho_{i_r}, \rho_{{i+1}_r}, s^j_r, s^{2.j}_r, b_{0_{blinding}} \in \mathbb{Z}_p$ and creates:
   $$
   T_{State_{old_r}} = bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + \rho_{i_r}.G_5 + s^j_r.G_6  + b_{0_{blinding}}.H_0, \in \mathbb{G}_p
   $$
   
   $$
   T_{State_{new}} = bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + \rho_{{i+1}_r}.G_5 + s^{2.j}_r.G_6, \in \mathbb{G}_p
   $$
...
11. Prover now creates responses for sigma protocol for $resp_{state_{old_r}}$ =
    $$[\begin{matrix}
    bal_{0_r} + bal_0.c, & cnt_r + cnt.c, & \rho_r + \rho.c, & \rho_{i_r} + \rho_i.c, & s^j_r + s^j.c, & b_{0_{blinding}} + b_0.c\\
    \end{matrix}]$$
12. Responses for sigma protocol for $resp_{state_{new}}$ =
    $$[\begin{matrix}
    \bot, & \bot, & \bot, & \rho_{{i+1}_r} + \rho_{i+1}.c, & s^{2.j}_r + s^{2.j}.c\\
    \end{matrix}]$$
...
4. Verifies correctness of $State_{old_r}$ by checking:
   ...
   $$resp_{state_{old_r}}[4].G_6 + resp_{state_{old_r}}[5].H_0 \stackrel{?}{=} T_{State_{old_r}} + (State_{old_r} - AK - at.G_3 - N - id.G_7).c$$

   $$\implies (bal_{0_r} + bal_0.c).G_1 + (cnt_r + cnt.c).G_2 + (\rho_r + \rho.c).G_4 + (\rho^i_r + \rho^i.c).G_5 + $$
   $$(s^j_r + s^j.c).G_6 + (b_{0_{blinding}} + b_0.c).H_0 \stackrel{?}{=} bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + \rho^i_r.G_5 + s^j_r.G_6  + b_{0_{blinding}}.H_0 + (State_{old_r} - AK - at.G_3 - N - id.G_7).c$$
...
5. Verifies correctness of $State_{new}$ by checking:
   ...
   $$resp_{state_{new}}[4].G_6 \stackrel{?}{=} T_{State_{new}} + (State_{new} - v.G_1 - AK - at.G_3 - id.G_7).c$$
   $$\implies (bal_{0_r} + bal_0.c).G_1 + (cnt_r + cnt.c).G_2 + (\rho_r + \rho.c).G_4 + (\rho^{i+1}_r + \rho^{i+1}.c).G_5 + (s^{2.j}_r + $$
   $$s^{2.j}.c).G_6 \stackrel{?}{=} bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + \rho^{i+1}_r.G_5 + s^{2.j}_r.G_6 + (State_{new} - v.G_1 - AK - at.G_3 - id.G_7).c$$
...
8. Verifier checks response for $C_{BP_{\rho,s}}$ as:
   ...
   $$(s^j_r + s^j.c).H_4 + (s^{2.j}_r + s^{2.j}.c).H_5 \stackrel{?}{=} r_{BP_{\rho,s}}.H_0 + \rho_r.H_1 + \rho^i_r.H_2 + \rho^{i+1}_r.H_3 + s^j_r.H_4 + s^{2.j}_r.H_5 + C_{BP_{\rho,s}}.c$$
