...
1. $State_{old} = sk.G_{Aff} + bal_0.G_1 + cnt.G_2 + at.G_3 + \rho.G_4 + \rho^i.G_5 + s^j.G_6 + id.G_7 + ek^{-1}.G_{Enc}$. Since public key $AK$ and nullifier $N$ are revealed to the verifier, this is equivalent to proving $bal_0.G_1 + cnt.G_2 + \rho.G_4 + \rho^i.G_5 + s^j.G_6 + ek^{-1}.G_{Enc} + b_0.H_0 = State_{old_r} - AK - at.G_3 - N - id.G_7$
2. Similarly, $State_{new} = AK + bal_1.G_1 + cnt.G_2 + at.G_3 + \rho.G_4 + \rho^{i+1}.G_5 + s^{2.j}.G_6 + id.G_7 + ek^{-1}.G_{Enc}$. Since $bal_1 = bal_0 + v$, this is equivalent to $State_{new} - v.G_1 - AK - at.G_3 - id.G_7 = bal_0.G_1 + cnt.G_2 + \rho.G_4 + \rho^{i+1}.G_5 + s^{2.j}.G_6 + ek^{-1}.G_{Enc}$
...
**Instance**: $State_{old_r}, State_{new}, AK, at, N, id, v, Path_r, Root, G_{Aff}, G_i, H_i, \widetilde{G}, \widetilde{G}_i, \widetilde{H}, \widetilde{H}_i$

**Witness**: $sk, bal_0, bal_1, cnt, \rho, \rho^i, \rho^{i+1}, s^j, s^{2.j}, ek^{-1}, b_0, Path$.
...
### Prover
1. Let $D_{old} = State_{old_r} - AK - at.G_3 - N - id.G_7$ (everything subtracted is public to the verifier). Prover wants to prove knowledge of $bal_0, cnt, \rho, \rho^i, s^j, ek^{-1}, b_0$ in $bal_0.G_1 + cnt.G_2 + \rho.G_4 + \rho^i.G_5 + s^j.G_6 + ek^{-1}.G_{Enc} + b_0.H_0 = D_{old}$
2. Similarly, let $D_{new} = State_{new} - v.G_1 - AK - at.G_3 - id.G_7$. Prover wants to prove knowledge of $bal_0, cnt, \rho, \rho^{i+1}, s^{2.j}, ek^{-1}$ in $bal_0.G_1 + cnt.G_2 + \rho.G_4 + \rho^{i+1}.G_5 + s^{2.j}.G_6 + ek^{-1}.G_{Enc} = D_{new}$.
...
5. Since prover knows the opening of $State_{old}$ and $b_0$, it can prove the knowledge of opening of $State_{old_r}$ as well. So relation 1 can be transformed as
   $$
   bal_0.G_1 + cnt.G_2 + \rho.G_4 + \rho^i.G_5 + s^j.G_6 + ek^{-1}.G_{Enc} + b_0.H_0 = State_{old_r} - AK - at.G_3 - N - id.G_7
   $$
   where the RHS is public. Now it starts proving the knowledge of these.
6. Prover picks random $sk_r, cnt_r, bal_{0_r}, \rho_r, \rho_{i_r}, \rho_{{i+1}_r}, s^j_r, s^{2.j}_r, ek^{-1}_r, b_{0_{blinding}} \in \mathbb{Z}_p$ and creates:
   $$
   T_{State_{old_r}} = bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + \rho_{i_r}.G_5 + s^j_r.G_6 + ek^{-1}_r.G_{Enc} + b_{0_{blinding}}.H_0, \in \mathbb{G}_p
   $$
   
   $$
   T_{State_{new}} = bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + \rho_{{i+1}_r}.G_5 + s^{2.j}_r.G_6 + ek^{-1}_r.G_{Enc}, \in \mathbb{G}_p
   $$
...
11. Prover now creates responses for sigma protocol for $resp_{state_{old_r}}$ =
    $$[\begin{matrix}
    bal_{0_r} + bal_0.c, & cnt_r + cnt.c, & \rho_r + \rho.c, & \rho_{i_r} + \rho_i.c, & s^j_r + s^j.c, & ek^{-1}_r + ek^{-1}.c, & b_{0_{blinding}} + b_0.c\\
    \end{matrix}]$$
12. Responses for sigma protocol for $resp_{state_{new}}$ =
    $$[\begin{matrix}
    \bot, & \bot, & \bot, & \rho_{{i+1}_r} + \rho_{i+1}.c, & s^{2.j}_r + s^{2.j}.c, & \bot\\
    \end{matrix}]$$
    
    The symbol $\bot$ indicates that no response is created for that witness since a response has been created for the same witness above. Because responses for $bal_0, cnt$, $\rho$ and $ek^{-1}$ are already created in $resp_{state_{old_r}}$.
...
4. Verifies correctness of $State_{old_r}$ by checking:
   ...
   $$resp_{state_{old_r}}[4].G_6 + resp_{state_{old_r}}[5].G_{Enc} + resp_{state_{old_r}}[6].H_0 \stackrel{?}{=} T_{State_{old_r}} + (State_{old_r} - AK - at.G_3 - N - id.G_7).c$$

   $$\implies (bal_{0_r} + bal_0.c).G_1 + (cnt_r + cnt.c).G_2 + (\rho_r + \rho.c).G_4 + (\rho^i_r + \rho^i.c).G_5 + $$
   $$(s^j_r + s^j.c).G_6 + (ek^{-1}_r + ek^{-1}.c).G_{Enc} + (b_{0_{blinding}} + b_0.c).H_0 \stackrel{?}{=} bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + $$
   $$\rho^i_r.G_5 + s^j_r.G_6 + ek^{-1}_r.G_{Enc} + b_{0_{blinding}}.H_0 + (State_{old_r} - AK - at.G_3 - N - id.G_7).c$$
...
5. Verifies correctness of $State_{new}$ by checking:
   ...
   $$resp_{state_{new}}[4].G_6 + resp_{state_{old_r}}[5].G_{Enc} \stackrel{?}{=} T_{State_{new}} + (State_{new} - v.G_1 - AK - at.G_3 - id.G_7).c$$
   $$\implies (bal_{0_r} + bal_0.c).G_1 + (cnt_r + cnt.c).G_2 + (\rho_r + \rho.c).G_4 + (\rho^{i+1}_r + \rho^{i+1}.c).G_5 + (s^{2.j}_r + $$
   $$s^{2.j}.c).G_6 + (ek^{-1}_r + ek^{-1}.c).G_{Enc} \stackrel{?}{=} bal_{0_r}.G_1 + cnt_r.G_2 + \rho_r.G_4 + \rho^{i+1}_r.G_5 + s^{2.j}_r.G_6 + ek^{-1}_r.G_{Enc} + (State_{new} - v.G_1 - AK - at.G_3 - id.G_7).c$$

   Note that for the $bal_0, cnt$, $\rho$ and $ek^{-1}$ witnesses, verifier reuses responses from $resp_{state_{old_r}}$.
...
8. Verifier checks response for $C_{BP_{\rho,s}}$ as:
   ...
   $$resp_{state_{old_r}}[4].H_4 + resp_{state_{new}}[4].H_5 \stackrel{?}{=} T_{BP_{\rho,s}} + C_{BP_{\rho,s}}.c$$
   $$\implies (r_{BP_{\rho,s}} + b'.c).H_0 + (\rho_r + \rho.c).H_1 + (\rho^i_r + \rho^i.c).H_2 + (\rho^{i+1}_r + \rho^{i+1}.c).H_3 + $$
   $$(s^j_r + s^j.c).H_4 + (s^{2.j}_r + s^{2.j}.c).H_5 \stackrel{?}{=} r_{BP_{\rho,s}}.H_0 + \rho_r.H_1 + \rho^i_r.H_2 + $$
   $$\rho^{i+1}_r.H_3 + s^j_r.H_4 + s^{2.j}_r.H_5 + C_{BP_{\rho,s}}.c$$
