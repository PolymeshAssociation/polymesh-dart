...
4. Verifies correctness of $State_{old_r}$ by checking:
   ...
   $$\implies (bal_{0_r} + bal_0.c).G_1 + (cnt_r + cnt.c).G_2 + (\rho_r + \rho.c).G_4 + (\rho_{i_r} + \rho_i.c).G_5 + $$
   $$(s^j_r + s^j.c).G_6 + (b_{0_{blinding}} + b_0.c).H_0 \stackrel{?}{=} T_{State_{old_r}} + (State_{old_r} - AK - at.G_3 - N - id.G_7).c$$
...
5. Verifies correctness of $State_{new}$ by checking:
   ...
   $$\implies (bal_{0_r} + bal_0.c).G_1 + (cnt_r + cnt.c).G_2 + (\rho_r + \rho.c).G_4 + (\rho_{{i+1}_r} + \rho_{i+1}.c).G_5 + (s^{2.j}_r + $$
   $$s^{2.j}.c).G_6 \stackrel{?}{=} T_{State_{new}} + (State_{new} - v.G_1 - AK - at.G_3 - id.G_7).c$$
...
8. Verifier checks response for $C_{BP_{\rho,s}}$ as:
   ...
   $$\implies (r_{BP_{\rho,s}} + b'.c).H_0 + (\rho_r + \rho.c).H_1 + (\rho_{i_r} + \rho_i.c).H_2 + (\rho_{{i+1}_r} + \rho_{i+1}.c).H_3 + $$
   $$(s^j_r + s^j.c).H_4 + (s^{2.j}_r + s^{2.j}.c).H_5 \stackrel{?}{=} T_{BP_{\rho,s}} + C_{BP_{\rho,s}}.c$$
