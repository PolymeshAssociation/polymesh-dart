...
4. Verifies correctness of $State_{old_r}$ by checking:
   ...
   $$\implies (bal_{0_{blinding}} + bal_0.c).G_1 + (cnt_{blinding} + cnt.c).G_2 + (\rho_{blinding} + \rho.c).G_4 + (\rho_{i_{blinding}} + \rho_i.c).G_5 + $$
   $$(s^j_{blinding} + s^j.c).G_6 + (b_{0_{blinding}} + b_0.c).H_0 \stackrel{?}{=} T_{State_{old_r}} + (State_{old_r} - AK - at.G_3 - N - id.G_7).c$$
...
5. Verifies correctness of $State_{new}$ by checking:
   ...
   $$\implies (bal_{0_{blinding}} + bal_0.c).G_1 + (cnt_{blinding} + cnt.c).G_2 + (\rho_{blinding} + \rho.c).G_4 + (\rho_{i+1_{blinding}} + \rho_{i+1}.c).G_5 + (s^{2.j}_{blinding} + $$
...
8. Verifier checks response for $C_{BP_{\rho,s}}$ as:
   ...
   $$\implies (r_{BP_{\rho,s}} + b'.c).H_0 + (\rho_{blinding} + \rho.c).H_1 + (\rho_{i_{blinding}} + \rho_i.c).H_2 + (\rho_{i+1_{blinding}} + \rho_{i+1}.c).H_3 + $$
   $$(s^j_{blinding} + s^j.c).H_4 + (s^{2.j}_{blinding} + s^{2.j}.c).H_5 \stackrel{?}{=} T_{BP_{\rho,s}} + C_{BP_{\rho,s}}.c$$
