...
However, sender and receiver do have an option to send an irreversible affirmation and in that case, they don't change their counter.

Table describing changes to balance and counter for various txns. Subscripts $_s$ and $_r$ refer to txn sent by sender and receiver respectively.
...
9. Since the change to counter is public, relation for $State_{new}$ can be expressd as:
...
4. Since prover knows the opening of $State_{old}$ and $b_0$, it can prove the knowledge of opening of $State_{old_r}$ as well. So relation 1 can be transformed as:
   ...
   $$
5. Prover picks random $$sk_r, at_r,  bal_{0_r},  cnt_r, \rho_r, \rho_{i_r}, \rho_{{i+1}_r}, s^j_r, s^{2.j}_r, id_r, b_{0_r} \in \mathbb{Z}_p$$ and creates:
   $$T_{State_{old_r}} = sk_r.G_{Aff} +  bal_{0_r}.G_1 +  cnt_r.G_2 + at_r.G_3 + \rho_r.G_4 + $$
   $$\rho_{i_r}.G_5 + s^j_r.G_6 + id_r.G_7 + b_{0_r}.H_0, \in \mathbb{G}_p$$
...
16. Prover creates responses for $resp_{state_{old_r}}$ =
    $$[\begin{matrix}
    sk_r + sk.c, &  bal_{0_r} + bal_0.c, &  cnt_r + cnt_0.c, & at_r + at.c, & \rho_r + \rho.c, & \rho_{i_r} + \rho_i.c, & s^j_r + s^j.c, & id_r + id.c, & b_{0_r} + b_0.c\\
    \end{matrix}]$$
...
$$(id_r + id.c).G_7 \stackrel{?}{=} T_{State_{new}} + (State_{new} + G_2).c$$
...
10. Verifier checks response for $C_{BP_{bal}}$ as:
    ...
    $$\implies (r_{BP_{bal}} + b''.c).H_0 + (v_r + v.c).H_1 + ( bal_{0_r} + bal_0.c).H_2 + ( bal_{1_r} + bal_1.c).H_3 \stackrel{?}{=} T_{BP_{bal}} + C_{BP_{bal}}.c$$
...
12. Verifier checks response for public key in leg by checking:
    ...
    $$\implies (r_{pk_r} + r_{pk}.c).G_{Enc} + (sk_r + sk.c).G_{Aff} \stackrel{?}{=} T_{pk} + CT_s.c$$

    $resp_{pk}.G_{Enc} + resp_{state_{old_r}}[0].G_{Aff} \stackrel{?}{=} T_{pk} + CT_r.c \text{ (if receiver)}$
    $\implies (r_{pk_r} + r_{pk}.c).G_{Enc} + (sk_r + sk.c).G_{Aff} \stackrel{?}{=} T_{pk} + CT_r.c$
