pub mod human_hex {
    ...
    impl<L> SerializeHex for BoundedVec<u8, L> {
        fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: ser::Serializer,
        {
    ...
    impl<'de, L: Get<u32>> DeserializeHex<'de> for BoundedVec<u8, L> {
        fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
        where
            D: de::Deserializer<'de>,
        {
            if deserializer.is_human_readable() {
                ...
                let vec = deserializer.deserialize_str(StringVisitor)?;
                vec.try_into()
                    .map_err(|_| de::Error::custom("byte array exceeds maximum allowed length"))
            } else {
                de::Deserialize::deserialize(deserializer)
            }
        }
